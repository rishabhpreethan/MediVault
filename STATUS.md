...
| MV-148 | Family tree connected-member state — differentiate Linked Account (accepted invite, solid node + "Linked" badge) vs Managed Profile (solid node + "Managed" badge) vs Pending (dashed node); update FamilyCirclePage NodeCard rendering | P2 | Done | Developer Agent | MV-134 | feature/MV-148-149-quick-wins |
| MV-149 | Document upload Coming Soon gate — replace UploadModal trigger with a Coming Soon modal/toast; keep all backend pipeline code intact; add "Notify me" CTA placeholder | P1 | Done | Developer Agent | — | feature/MV-148-149-quick-wins |

### EPIC: User Onboarding (DECISION-008)
